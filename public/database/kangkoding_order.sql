...
--
-- Host: localhost
-- Generation Time: May 01, 2018 at 07:48 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_BANK_ACCOUNT` (IN `_owner` INT)  NO SQL
Select
	A.Id,
	A.AccountName,
    A.AccountNumber,
...
From 
	BankAccount as A
LEFT JOIN
	_Bank as B ON A._Bank = B.Id
ORDER BY
	A.UpdatedDt$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_ORDER_DETAIL` (IN `_orderCode` VARCHAR(20))  NO SQL
SELECT
	A.OrderCode,
	A._Account AS 'UserId',
    A.Fullname,
    A._Province AS 'ProvinceId',
    H.Name AS 'ProvinceName',
    A._City AS 'CityId',
    I.Name AS 'CityName',
    A._District AS 'DistrictId',
    ...
    D.Size AS 'ProductSize',
    B.Total AS 'ProductTotal',
    B.Price AS 'ProductPrice',
    B.TotalPrice AS 'ProductTotalPrice',
    CONCAT_WS('/',F.Uuid,CONCAT_WS('/',P.Uuid,E.Path)) AS 'ProductPhoto'
FROM
	PurchaseOrder AS A
LEFT JOIN
	PurchaseOrderDetail AS B ON A.OrderCode = B._PurchaseOrder
LEFT JOIN
	Product AS P ON B._Product = P.Id
LEFT JOIN
	_Color AS C ON C.Id = B._Color
LEFT JOIN
	_Size AS D ON D.Id = B._Size
INNER JOIN
	Gallery AS E ON B._Product = E._Product AND B._Color = E._Color
INNER JOIN
	Owner AS F ON P._Owner = F.Id
LEFT JOIN
	_Province AS H ON A._Province = H.Id
LEFT JOIN
	_City AS I ON A._City = I.Id
LEFT JOIN
	_District AS J ON A._District = J.Id
WHERE
	A.OrderCode = _orderCode
GROUP BY E._Color
ORDER BY B._Product ASC$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_ORDER_LIST` (IN `_owner` INT, IN `_orderBy` INT, IN `_search` VARCHAR(100), IN `_status` INT, IN `_account` INT)  NO SQL
SELECT SQL_CALC_FOUND_ROWS
	FOUND_ROWS() as "TotalData",
	A.OrderCode,
	A._Account AS 'UserId',
    A.Fullname,
    A._Province AS 'ProvinceId',
...
WHERE
	A._Owner = _owner AND
    (CASE WHEN _search IS NOT NULL THEN A.Fullname LIKE CONCAT('%', _search , '%') END) AND
	(CASE WHEN _account = 0 THEN A._Account IS NOT NULL WHEN _account != 0 THEN A._Account = _account END) AND
	(CASE WHEN _status = 6 THEN A.Status IS NOT NULL WHEN _status != 6 THEN A.Status = _status END)
ORDER BY
	CASE WHEN _orderBy = 0 THEN A.CreatedDt END DESC,
	CASE WHEN _orderBy = 1 THEN A.CreatedDt END ASC$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_PRODUCT` (IN `_owner` INT, IN `_name` VARCHAR(100), IN `_category` VARCHAR(255), IN `_gender` VARCHAR(10), IN `_priceBot` INT, IN `_priceTop` INT, IN `_limit` INT, IN `_limitStart` INT, IN `_orderBy` INT, IN `_color` VARCHAR(255))  NO SQL
SELECT SQL_CALC_FOUND_ROWS
	FOUND_ROWS() as "TotalData",
	A.Id,
    F._Product AS 'isPhoto',
    CONCAT_WS('/',G.Uuid,CONCAT_WS('/',A.Uuid,F.Path)) AS 'Photo',
	A.Name,
	A.Description,
	B.Name AS 'Category',
	C.Name AS 'Gender',
	A.Price AS 'oldPrice',
    (CASE
		WHEN E.Type IS NULL THEN
     		A.PRICE
     	WHEN E.Id = 1 THEN
     		A.Price - ((D.Value / 100) * A.Price)
     	WHEN E.Id = 2 THEN
     		A.Price - D.Value
    END) AS 'newPrice',
	E.Type AS 'DiscountType',
    D.Value AS 'Discount'
FROM
	Product AS A
LEFT JOIN
	_Category AS B ON A._Category = B.Id
LEFT JOIN
	_Gender AS C ON A._Gender = C.Id
LEFT JOIN
	Stock AS H ON A.Id = H._Product
LEFT JOIN
	(SELECT Id,_Product,_DiscountType,Value FROM Discount WHERE StartDt < NOW() AND EndDt > NOW()) AS D ON A.Id = D._Product
LEFT JOIN
	_DiscountType AS E ON D._DiscountType = E.Id
LEFT JOIN
	(SELECT _Product,Path FROM Gallery WHERE Selected = 1) AS F ON A.Id = F._Product
INNER JOIN
	Owner AS G ON A._Owner = G.Id
WHERE
	G.Id = _owner AND
	(CASE WHEN _name IS NOT NULL THEN A.Name LIKE CONCAT('%', _name , '%') END) AND
    (CASE WHEN _category = 0 THEN B.Id IS NOT NULL AND B._Owner = _owner WHEN _category != 0 THEN find_in_set(B.Id, _category) AND B._Owner = _owner END) AND
    (CASE WHEN _gender = 0 THEN C.Id IS NOT NULL WHEN _gender != 0 THEN find_in_set(C.Id, _gender) END) AND
    (CASE WHEN _color = 0 THEN H.Id IS NOT NULL WHEN _color != 0 THEN find_in_set(H._Color, _color) END) AND
	(CASE WHEN _priceBot = 0 THEN A.Price >= 0 WHEN _priceBot != 0 THEN A.Price >= _priceBot END) AND 
	(CASE WHEN _priceTop = 0 THEN A.Price >= 0 WHEN _priceTop != 0 THEN A.Price <= _priceTop END)
GROUP BY A.id
ORDER BY
	CASE WHEN _orderBy = 0 THEN A.CreatedDt END DESC,
	CASE WHEN _orderBy = 1 THEN A.CreatedDt END ASC,
	CASE WHEN _orderBy = 2 THEN A.Price END DESC,
	CASE WHEN _orderBy = 3 THEN A.Price END ASC
LIMIT
	_limitStart,_limit$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_PRODUCT_DETAIL` (IN `_owner` INT, IN `_productId` INT)  NO SQL
SELECT 
	A.Id,
	A.Name, 
	A.Description,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_PRODUCT_DETAIL_LIST` (IN `_owner` INT, IN `_productIdList` VARCHAR(255))  NO SQL
SELECT 
	H.Id,
	H.Name,
    E.Type AS 'DiscountType',
    ...
    (CASE
		WHEN E.Type IS NULL THEN
     		H.PRICE
     	WHEN E.Id = 1 THEN
     		H.Price - ((D.Value / 100) * H.Price)
     	WHEN E.Id = 2 THEN
     		H.Price - D.Value
    END) AS 'newPrice'
    FROM Stock as A
LEFT JOIN
	Product AS H ON H.Id = A._Product
LEFT JOIN
	(SELECT Id,_Product,_DiscountType,Value FROM Discount WHERE StartDt < NOW() AND EndDt > NOW()) AS D ON H.Id = D._Product
LEFT JOIN
	_DiscountType AS E ON D._DiscountType = E.Id
LEFT JOIN
	Owner AS G ON H._Owner = G.Id
LEFT JOIN
	_Color AS I ON A._Color = I.Id
LEFT JOIN
	_Size AS J ON A._Size = J.Id
WHERE
    G.Id = _owner AND
    find_in_set(A._Product, _productIdList)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_PRODUCT_DETAIL_PHOTO` (IN `_owner` INT, IN `_productId` INT)  NO SQL
SELECT
	A.Id,
    A._Product,
    A._Color,
   	CONCAT_WS('/',C.Uuid,CONCAT_WS('/',B.Uuid,A.Path)) AS 'Photo',
    A.Description,
    A.Selected
	FROM Gallery AS A
LEFT JOIN 
	Product as B ON A._Product = B.Id
LEFT JOIN
	Owner as C ON B._Owner = C.Id
WHERE 
	A._Product = _productId AND
	C.Id = _owner$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_PRODUCT_DETAIL_PHOTO_COLOR` (IN `_owner` INT, IN `_productId` INT, IN `_color` INT)  NO SQL
SELECT
	A.Id,
    A._Product,
    A._Color,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_PRODUCT_DETAIL_STOCK` (IN `_owner` INT, IN `_productId` INT)  NO SQL
SELECT
	A.Total,
    B.Id as ColorId,
    B.Name as ColorName,
    ...
    FROM Stock AS A
LEFT JOIN
	_Color AS B ON B.Id = A._Color
LEFT JOIN
	_Size AS C ON C.Id = A._Size
LEFT JOIN
	Product as D ON D.Id = A._Product
WHERE
	A._Product = _productId AND
    D._Owner = _owner$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_PRODUCT_LIST` (IN `_owner` INT, IN `_name` VARCHAR(100), IN `_category` VARCHAR(255), IN `_gender` VARCHAR(10), IN `_priceBot` INT, IN `_priceTop` INT, IN `_limit` INT, IN `_limitStart` INT, IN `_orderBy` INT)  NO SQL
SELECT SQL_CALC_FOUND_ROWS
	FOUND_ROWS() as "TotalData",
	A.Id,
    F._Product AS 'isPhoto',
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_STOCK` (IN `_owner` INT, IN `_orderBy` INT, IN `_limit` INT, IN `_limitStart` INT)  NO SQL
SELECT SQL_CALC_FOUND_ROWS
	FOUND_ROWS() as "TotalData",
    B.Id,
	A.Id AS 'ProductId',
...
FROM
	Product AS A
LEFT JOIN
	Stock AS B ON A.Id = B._Product
LEFT JOIN
	_Color AS C ON B._Color = C.Id
LEFT JOIN
	_Size AS D ON B._Size = D.Id
INNER JOIN
	Owner AS G ON A._Owner = G.Id
WHERE
	G.Id = _owner
GROUP BY A.id
ORDER BY
	CASE WHEN _orderBy = 0 THEN B.CreatedDt END DESC,
	CASE WHEN _orderBy = 1 THEN B.CreatedDt END ASC
LIMIT
	_limitStart,_limit$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_USER_ADDRESS` (IN `_account` INT)  NO SQL
SELECT
	A.Id,
    A.Name,
    A.RecipientName,
...
FROM
	Address as A
LEFT JOIN
	_Province AS B ON A._Province = B.Id
LEFT JOIN
	_City AS C ON A._City = C.Id
LEFT JOIN
	_District AS D ON A._District = D.Id
WHERE
	A._Account = _account
ORDER BY A.UpdatedDt DESC$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `INSERT_PRODUCT` (IN `Owner` INT, IN `Name` VARCHAR(100), IN `Description` VARCHAR(500), IN `Category` INT, IN `Gender` INT, IN `Price` DOUBLE, IN `CreatedDt` TIMESTAMP, IN `CreatedBy` INT)  NO SQL
INSERT INTO Product
(_Owner,Name,Description,_Category,_Gender,Price,CreatedDt,CreatedBy)
VALUES
(Owner,Name,Description,Category,Gender,Price,CreatedDt,CreatedBy)$$
...
CREATE TABLE `Account` (
  ...
  `isForgot` tinyint(1) DEFAULT NULL,
  `isForgotToken` varchar(6) DEFAULT NULL,
  `isForgotExpired` timestamp NULL DEFAULT NULL,
...
INSERT INTO `Account` (`Id`, `_Owner`, `_Role`, `Email`, `Fullname`, `Password`, `Phone`, `isForgot`, `isForgotToken`, `isForgotExpired`, `LastLogin`, `CreatedDt`, `UpdatedDt`, `UpdatedBy`, `Status`) VALUES
(1, 1, 2, 'owner_one@gmail.com', 'Joko Sutirman', '$2y$10$KeyKJX37jYmI6ai.rX.tmuDFlTuheagTX7bSNvJ5SXZN3uUxQ1w6.', NULL, NULL, NULL, NULL, '0000-00-00 00:00:00', '0000-00-00 00:00:00', NULL, NULL, 1),
(6, 1, 4, 'anton.paijol@gmail.com', 'Anton Paijol', '$2y$10$lU6S8F14vhoNh00nm8MD4.9DtW4/Fm74XrUqswHuwOIhVrvA/SKKW', NULL, NULL, NULL, NULL, '2018-03-29 06:04:53', '2018-03-29 06:04:53', NULL, NULL, 1),
(7, 1, 4, 'user.hadron@gmail.com', 'Hadron Megantara User', '$2y$10$8Y8Pczgt38TorJVtaeT3uuL4CicyRCE6nFxVHBtsqTMp2DV2Kgszm', NULL, NULL, NULL, NULL, '2018-04-03 15:33:03', '2018-04-03 15:33:03', NULL, NULL, 1),
(8, 1, 4, 'hadron.user.satu@gmail.com', 'Hadron user satu', '$2y$10$lU6S8F14vhoNh00nm8MD4.9DtW4/Fm74XrUqswHuwOIhVrvA/SKKW', '085691108802', NULL, NULL, NULL, '2018-04-03 15:44:36', '2018-04-03 15:44:36', NULL, NULL, 1),
(9, 1, 3, 'admin.ownersatu@gmail.com', 'admin satu', '$2y$10$lU6S8F14vhoNh00nm8MD4.9DtW4/Fm74XrUqswHuwOIhVrvA/SKKW', NULL, NULL, NULL, NULL, NULL, '0000-00-00 00:00:00', NULL, NULL, 0),
(10, 1, 4, 'user1@gmail.com', 'User 1', '$2y$10$bh2K31ElsQOnNluiRMuUK.esAaTx7jz.n71IogPGpjLHluEPk9EV6', NULL, NULL, NULL, NULL, '2018-04-25 07:46:11', '2018-04-25 07:46:11', NULL, NULL, 1),
(11, 1, 4, 'user1@gmail.com', 'User 1', '$2y$10$skDalgDzUxYiVB53zFWgHeJOKmxSKBp90lrZkcADzKJsDXT0rf02a', NULL, NULL, NULL, NULL, '2018-04-25 07:46:16', '2018-04-25 07:46:16', NULL, NULL, 1),
(12, 1, 4, 'user1@gmail.com', 'User 1', '$2y$10$B9krPE/PKLC1t61pzGuE8.5IzS/JuHpWYEiOg03tdZO019Lum3OzC', NULL, NULL, NULL, NULL, '2018-04-25 07:46:20', '2018-04-25 07:46:20', NULL, NULL, 1),
(13, 1, 4, 'user1@gmail.com', 'User 1', '$2y$10$fnU739L70ewf6wXUKGN.OuzQ1EqWvq6nyI8ADRLi/k1Rm/ulwoYvC', NULL, NULL, NULL, NULL, '2018-04-25 07:46:21', '2018-04-25 07:46:21', NULL, NULL, 1),
(14, 1, 4, 'user1@gmail.com', 'User 1', '$2y$10$gko8xK7lmhKQAtRB9gmrg.83r44/gK51QWEyU3agmPg7ACNpoDT1a', NULL, NULL, NULL, NULL, '2018-04-25 07:46:22', '2018-04-25 07:46:22', NULL, NULL, 1),
(15, 1, 4, 'user3@gmail.com', 'User 3', '$2y$10$vyixcYUqtR3oSeSLxGoiFuKaepoGj.RrFx4Set6lERMROgtoe5lIe', NULL, NULL, NULL, NULL, '2018-04-25 07:53:22', '2018-04-25 07:53:22', NULL, NULL, 1),
(16, 1, 4, 'user3@gmail.com', 'User 3', '$2y$10$MiJ/gjIU82dRN5bzwtaxxuRdbNutXznflbxy4hIsveq392ujRE6g.', NULL, NULL, NULL, NULL, '2018-04-25 07:54:21', '2018-04-25 07:54:21', NULL, NULL, 1),
(17, 1, 4, 'data@gmail.com', 'data', '$2y$10$v4owlIBaCtgIPLpmxrOA7OrmS8dlQqmYgcOxouJoA8SQ5EHQISKBe', NULL, NULL, NULL, NULL, '2018-04-25 07:56:53', '2018-04-25 07:56:53', NULL, NULL, 1);
...
CREATE TABLE `Address` (
  ...
  `_Province` int(11) NOT NULL,
  `_City` int(11) NOT NULL,
  `_District` int(11) NOT NULL,
...
INSERT INTO `Address` (`Id`, `_Account`, `_Province`, `_City`, `_District`, `RecipientName`, `RecipientPhone`, `Name`, `Address`, `PostCode`, `CreatedDt`, `UpdatedDt`) VALUES
(1, 8, 31, 3171, 3171100, 'Hadron', '085716225637', 'Kosans', 'Jl KH Royani 1 No 5A', NULL, '2018-04-29 08:40:19', '2018-04-29 08:40:19'),
(2, 8, 32, 3201, 3201250, 'Grant', '081219298485', 'Rumah', 'Perum Bukit Dago Blok D4 No 23', NULL, '2018-04-29 08:29:45', '2018-04-29 08:29:45');
...
INSERT INTO `BankAccount` (`Id`, `_Owner`, `_Bank`, `AccountName`, `AccountNumber`, `Branch`, `CreatedDt`, `CreatedBy`, `UpdatedDt`, `UpdatedBy`) VALUES
(1, 1, 6, 'PT Kangkoding', '5223413121', 'Jakarta', '2018-04-23 09:01:09', 9, '2018-04-23 09:01:09', 9),
(2, 1, 2, 'PT Kangkoding', '1092039394', 'Jakarta', '2018-04-23 09:38:51', 9, '2018-04-23 09:38:51', 9);
...
CREATE TABLE `Cart` (
  `Id` int(11) NOT NULL,
  `_Account` int(11) NOT NULL,
  `_Product` int(11) NOT NULL,
  `_Color` int(11) NOT NULL,
...
CREATE TABLE `Discount` (
  ...
  `_DiscountType` tinyint(4) NOT NULL,
...
INSERT INTO `Discount` (`Id`, `_Product`, `StartDt`, `EndDt`, `_DiscountType`, `Value`, `CreatedDt`, `CreatedBy`, `UpdatedDt`, `UpdatedBy`) VALUES
(1, 2, '2018-04-20 21:19:28', '2018-04-29 17:00:00', 2, '10000', '2018-03-28 17:00:00', 1, '0000-00-00 00:00:00', 0),
(2, 8, '2018-04-20 17:00:00', '2018-04-29 17:00:00', 1, '10', '2018-03-28 17:00:00', 9, '2018-04-20 14:36:22', 9),
(24, 72, '2018-04-20 17:00:00', '2018-04-29 17:00:00', 1, '10', '2018-04-20 16:02:47', 9, '2018-04-20 16:02:47', 9),
(25, 73, '2018-04-21 17:00:00', '2018-05-31 16:00:00', 1, '10', '2018-04-22 05:09:50', 9, '2018-04-22 05:09:50', 9);
...
INSERT INTO `Gallery` (`Id`, `_Product`, `_Color`, `Path`, `Description`, `CreatedDt`, `CreatedBy`, `UpdatedDt`, `UpdatedBy`, `Selected`, `Status`) VALUES
(1, 1, 14, 'big_v1-62-42763-78384-11853-46-103453.jpg', 'keterangan 1', '2018-04-01 09:02:57', 0, '0000-00-00 00:00:00', 0, 1, 0),
(2, 2, 1, 'big_v1-426-38605-76268-11273-46-100417-2.jpg', 'Keterangan warna merah', '2018-03-31 22:14:06', 0, '0000-00-00 00:00:00', 0, 1, 0),
(3, 4, 14, 'big_v1-346-34903-78846-11967-46-104051-2.jpg', 'keterangan 3', '2018-04-02 10:30:17', 0, '0000-00-00 00:00:00', 0, 1, 0),
(4, 7, 3, 'big_v1-62-42547-77792-11664-46-102594-2.jpg', 'keterangan 4', '2018-04-02 10:30:54', 0, '0000-00-00 00:00:00', 0, 1, 0),
(5, 8, 5, 'big_v1-589-42026-76636-11374-46-100926.jpg', 'keterangan 5', '2018-04-02 10:31:51', 0, '0000-00-00 00:00:00', 0, 1, 0),
(6, 9, 2, 'big_v1-589-42031-76647-11375-46-100937.jpg', 'keterangan 6', '2018-04-02 10:33:36', 0, '0000-00-00 00:00:00', 0, 1, 0),
(7, 10, 5, 'big_v1-589-42025-76635-11374-46-100925.jpg', 'keterangan 7', '2018-04-02 10:33:40', 0, '0000-00-00 00:00:00', 0, 1, 0),
(8, 11, 18, 'big_v1-539-35593-62917-8113-46-82624-2.jpg', 'keterangan 8', '2018-04-02 10:34:14', 0, '0000-00-00 00:00:00', 0, 1, 0),
(9, 12, 2, 'big_v1-346-41231-74815-10921-46-98464.jpg', 'keterangan 9', '2018-04-02 10:34:15', 0, '0000-00-00 00:00:00', 0, 1, 0),
(10, 13, 3, 'big_v1-346-41230-74813-10921-46-98462.jpg', 'keterangan 10', '2018-04-02 10:34:44', 0, '0000-00-00 00:00:00', 0, 1, 0),
(11, 14, 6, 'big_v1-324-40584-73301-10583-46-96543-2.jpg', 'keterangan 11', '2018-04-02 10:34:46', 0, '0000-00-00 00:00:00', 0, 1, 0),
(12, 15, 14, 'big_v1-262-39662-72791--46-95700.jpg', 'keterangan 12', '2018-04-02 10:34:49', 0, '0000-00-00 00:00:00', 0, 1, 0),
(13, 2, 1, 'v1-426-38605-76268-11273-46-100417.jpg', 'Keterangan warna merah', '2018-03-31 22:14:25', 0, '0000-00-00 00:00:00', 0, 0, 0),
(14, 2, 12, 'v1-426-38605-69106-9712-46-90615-2.jpg', 'Keterangan warna lavender', '2018-03-31 22:16:32', 0, '0000-00-00 00:00:00', 0, 0, 0),
(15, 2, 6, 'v1-426-38605-69108-9712-46-90617-2.jpg', 'Keterangan warna hitam', '2018-03-31 22:16:42', 0, '0000-00-00 00:00:00', 0, 0, 0),
(16, 2, 10, 'v1-426-38605-69105-9712-46-90614-2.jpg', 'Keterangan warna milo', '2018-03-31 22:17:39', 0, '0000-00-00 00:00:00', 0, 0, 0),
(17, 2, 13, 'v1-426-38605-69107-9712-46-90616.jpg', 'Keterangan warna blue pastel', '2018-03-31 22:17:55', 0, '0000-00-00 00:00:00', 0, 0, 0),
(18, 16, 11, 'Malika-Khimar-Syari-Pink.jpg', 'Keterangan Malika Khimar Syar\'i Pink', '2018-04-02 10:16:24', 0, '0000-00-00 00:00:00', 0, 1, 0),
(19, 16, 6, 'Malika-Khimar-Syari-Black.jpg', 'Keterangan Malika Khimar Syar\'i Hitam', '2018-04-02 09:50:22', 0, '0000-00-00 00:00:00', 0, 0, 0),
(20, 16, 3, 'Malika-Khimar-Syari-Purple.jpg', 'Keterangan Malika Khimar Syar\'i Ungu', '2018-04-02 09:50:49', 0, '0000-00-00 00:00:00', 0, 0, 0),
(21, 17, 15, 'Khimar-Syaimah-MintGreen.jpg', 'Keterangan Khimar Syaimah Mint Green', '2018-04-02 10:16:31', 0, '0000-00-00 00:00:00', 0, 1, 0),
(22, 17, 15, 'Khimar-Syaimah-MintGreen2.jpg', 'Keterangan Khimar Syaimah Mint Green2', '2018-04-02 09:54:51', 0, '0000-00-00 00:00:00', 0, 0, 0),
(23, 18, 16, 'Khimar-Aisyah2-BlueDenim.jpg', 'Keterangan Khimar Aisyah2 Blue Denim', '2018-04-02 10:16:36', 0, '0000-00-00 00:00:00', 0, 1, 0),
(24, 18, 16, 'Khimar-Aisyah2-BlueDenim2.jpg', 'Keterangan Khimar Aisyah Blue Denim2', '2018-04-02 09:57:12', 0, '0000-00-00 00:00:00', 0, 0, 0),
(25, 21, 17, 'Ayyasa-Khimar-Brown.jpg', 'Keterangan Ayyasa Khimar Brown', '2018-04-02 10:16:42', 0, '0000-00-00 00:00:00', 0, 1, 0),
(26, 21, 17, 'Ayyasa-Khimar-Brown2.jpg', 'Keterangan Ayyasa Khimar Brown2', '2018-04-02 09:59:37', 0, '0000-00-00 00:00:00', 0, 0, 0),
(27, 21, 18, 'Ayyasa-Khimar-Yellow.jpg', 'Keterangan Ayyasa Khimar Yellow', '2018-04-02 10:00:56', 0, '0000-00-00 00:00:00', 0, 0, 0),
(28, 21, 18, 'Ayyasa-Khimar-Yellow2.jpg', 'Keterangan Ayyasa Khimar Yellow2', '2018-04-02 10:01:11', 0, '0000-00-00 00:00:00', 0, 0, 0),
(29, 22, 12, 'Chesa-Khimar-Lavender.jpg', 'Keterangan Chesa Khimar Lavender', '2018-04-02 10:16:46', 0, '0000-00-00 00:00:00', 0, 1, 0),
(30, 22, 12, 'Chesa-Khimar-Lavender.jpg', 'Keterangan Chesa Khimar Lavender2', '2018-04-02 10:02:31', 0, '0000-00-00 00:00:00', 0, 0, 0),
(31, 22, 12, 'Chesa-Khimar-LightBlue.jpg', 'Keterangan Chesa Khimar Light Blue', '2018-04-02 10:02:52', 0, '0000-00-00 00:00:00', 0, 0, 0),
(32, 22, 12, 'Chesa-Khimar-LightBlue2.jpg', 'Keterangan Chesa Khimar LightBlue2', '2018-04-02 10:03:10', 0, '0000-00-00 00:00:00', 0, 0, 0),
(33, 23, 19, 'Afifa-Veil-DarkGrey.jpg', 'Keterangan Afifa Veil Dark Grey', '2018-04-02 10:16:49', 0, '0000-00-00 00:00:00', 0, 1, 0),
(34, 23, 19, 'Afifa-Veil-DarkGrey2.jpg', 'Keterangan Afifa Veil Dark Grey2', '2018-04-02 10:04:54', 0, '0000-00-00 00:00:00', 0, 0, 0),
(35, 24, 11, 'Zahra-Veil-Pink.jpg', 'Keterangan Zahra Veil Pink', '2018-04-02 10:16:54', 0, '0000-00-00 00:00:00', 0, 1, 0),
(36, 24, 11, 'Zahra-Veil-Pink2.jpg', 'Keterangan Zahra Veil Pink2', '2018-04-02 10:05:59', 0, '0000-00-00 00:00:00', 0, 0, 0),
(37, 25, 20, 'Azia-Veil-Grey.jpg', 'Keterangan Azia Veil Grey', '2018-04-02 10:16:57', 0, '0000-00-00 00:00:00', 0, 1, 0),
(38, 25, 20, 'Azia-Veil-Grey2.jpg', 'Keterangan Azia Veil Grey2', '2018-04-02 10:07:55', 0, '0000-00-00 00:00:00', 0, 0, 0),
(39, 26, 3, 'Saila-Veil-Purple.jpg', 'Keterangan Saila Veil Purple', '2018-04-02 10:17:01', 0, '0000-00-00 00:00:00', 0, 1, 0),
(40, 26, 3, 'Saila-Veil-Purple2.jpg', 'Keterangan Saila Veil Purple2', '2018-04-02 10:12:12', 0, '0000-00-00 00:00:00', 0, 0, 0),
(41, 26, 3, 'Saila-Veil-Purple3.jpg', 'Keterangan Saila Veil Purple3', '2018-04-02 10:12:01', 0, '0000-00-00 00:00:00', 0, 0, 0),
(42, 27, 11, 'Hania-Veil-Pink.jpg', 'Keterangan Hania Veil', '2018-04-02 10:17:05', 0, '0000-00-00 00:00:00', 0, 1, 0),
(43, 27, 11, 'Hania-Veil-Pink2.jpg', 'Keterangan Hania Veil Pink2', '2018-04-02 10:13:11', 0, '0000-00-00 00:00:00', 0, 0, 0),
(44, 28, 21, 'Arwa-Khimar-Maroon.jpg', 'Keterangan Arwa Khimar Maroon', '2018-04-02 10:17:08', 0, '0000-00-00 00:00:00', 0, 1, 0),
(55, 72, 17, 'AoaQmwhP7akoSt5VIyV7rlRru2QRaPeJaRPfGNhP-2018-04-20 23:02:47.jpeg', NULL, '2018-04-22 12:19:01', 9, '2018-04-20 16:02:47', 9, 1, 0),
(56, 72, 17, 'mRpH97KF4RvLiRc9ziFcWjyR08MZv0Ie3nyDpxPB-2018-04-20 23:02:48.jpeg', NULL, '2018-04-20 16:02:48', 9, '2018-04-20 16:02:48', 9, 0, 0),
(57, 72, 17, 'p71zDgkYDclnoGYhMfwxXLMwuKjRYRfeCjWedEEZ-2018-04-20 23:02:48.jpeg', NULL, '2018-04-20 16:02:48', 9, '2018-04-20 16:02:48', 9, 0, 0),
(58, 73, 6, 'Vu12TgCw3JpgljimOGbUrPRKxdBPd7qOUBtWDFlO-2018-04-22 12:09:50.jpeg', NULL, '2018-04-22 12:19:05', 9, '2018-04-22 05:09:50', 9, 1, 0),
(59, 73, 6, 'RZYY4ltCIAgW8Ew0wiRTXJGiEvtZlbcPVBAmD5Gk-2018-04-22 12:09:50.jpeg', NULL, '2018-04-22 05:09:50', 9, '2018-04-22 05:09:50', 9, 0, 0),
(60, 74, 11, 'HP3nqRpESzG4ACmyMkkm4VAVFBbKI1PG43UCPZ6H-2018-04-22 16:13:55.jpeg', NULL, '2018-04-22 16:15:15', 9, '2018-04-22 09:13:55', 9, 1, 0),
(61, 74, 11, 'iBF7fzJo9KkWuuMjUfxrVjI3opC4oLi3sSJWQiNT-2018-04-22 16:13:55.jpeg', NULL, '2018-04-22 09:13:55', 9, '2018-04-22 09:13:55', 9, 0, 0),
(62, 74, 11, 'o9cxKXOcwAXIgWmdAGW8G0HqWMaMnkfaNKycgUue-2018-04-22 16:13:55.jpeg', NULL, '2018-04-22 09:13:55', 9, '2018-04-22 09:13:55', 9, 0, 0),
(63, 74, 11, 'uH4jcNQVmQbMaky5cWp0NgZtKBYyR2eyq2bF8MXr-2018-04-22 16:13:56.jpeg', NULL, '2018-04-22 09:13:56', 9, '2018-04-22 09:13:56', 9, 0, 0),
(64, 75, 2, 'qb1tQqomDq2kkPjpNNR9TKFnRC5K3BkPzPgEvgN1-2018-04-22 16:16:50.jpeg', NULL, '2018-04-22 16:21:34', 9, '2018-04-22 09:16:50', 9, 1, 0),
(65, 75, 2, 'b6xD2T8hkrbXK6UaOdTXtKETf0TU0kITn21E5bwv-2018-04-22 16:16:51.jpeg', NULL, '2018-04-22 16:21:36', 9, '2018-04-22 09:16:51', 9, 0, 0),
(66, 75, 2, '9GOoICv1SOQFsXVqOL6htESDaC3HOqexbPk6QGkm-2018-04-22 16:16:51.jpeg', NULL, '2018-04-22 16:21:40', 9, '2018-04-22 09:16:51', 9, 0, 0),
(67, 76, 6, 'vpYRQgKGG6bTVM0uxgx8qRX9RJ4gN7rng6hPOMCE-2018-04-22 16:19:36.jpeg', NULL, '2018-04-22 16:22:48', 9, '2018-04-22 09:19:36', 9, 1, 0),
(68, 76, 6, '2tVfOK9uZyCbG20KEdf1HauPc3LNteZtzy3YpkRJ-2018-04-22 16:19:36.jpeg', NULL, '2018-04-22 09:19:36', 9, '2018-04-22 09:19:36', 9, 0, 0),
(69, 76, 6, 'jx3NlQ5bMiFk5JnTVF5CtVIllTmUmAnpDlkFVhmX-2018-04-22 16:19:37.jpeg', NULL, '2018-04-22 09:19:37', 9, '2018-04-22 09:19:37', 9, 0, 0),
(70, 77, 2, 'syDNWd16YriYBAIjYYmVPmvz0GAM9cgtkPOItwo0-2018-04-22 16:24:22.jpeg', NULL, '2018-04-22 16:24:44', 9, '2018-04-22 09:24:22', 9, 1, 0),
(71, 77, 2, 'As9uxNZXuNZcO76jaWson6WQr6JcoW37HQ27WQy3-2018-04-22 16:24:22.jpeg', NULL, '2018-04-22 09:24:22', 9, '2018-04-22 09:24:22', 9, 0, 0),
(72, 77, 2, 'zT8Ph5Fm3K21NyneJ3ShTjHXns77hGeAhvo0qf0H-2018-04-22 16:24:22.jpeg', NULL, '2018-04-22 09:24:22', 9, '2018-04-22 09:24:22', 9, 0, 0),
(73, 78, 1, 'Y5KVcBfXzfLKEwf4A3NaRhHCdH6jaBPVTsSStGWu-2018-04-25 22:29:21.jpeg', NULL, '2018-04-25 15:29:21', 9, '2018-04-25 15:29:21', 9, 0, 0),
(74, 78, 1, 'sTVzAwNiBUs50C60bPU4zqAH6w9H6Xu8qhmv20rF-2018-04-25 22:29:22.jpeg', NULL, '2018-04-25 15:29:22', 9, '2018-04-25 15:29:22', 9, 0, 0),
(75, 78, 1, 'DhljAteGeaOwinbdrDxJQiQO1ehE7LJRTCLHHsxw-2018-04-25 22:29:22.jpeg', NULL, '2018-04-25 15:29:22', 9, '2018-04-25 15:29:22', 9, 0, 0),
(76, 79, 18, 'jbpgsA2LpkBv8RMlOJrOncbelL3APm1uq311SHu3-2018-04-26 00:31:23.jpeg', NULL, '2018-04-25 17:31:23', 9, '2018-04-25 17:31:23', 9, 0, 0),
(77, 79, 18, '1WeaXAShY2DEdn6xVHFJPvX0O52gZ5MFyACMbczs-2018-04-26 00:31:24.jpeg', NULL, '2018-04-25 17:31:24', 9, '2018-04-25 17:31:24', 9, 0, 0),
(78, 80, 18, 'LSwDFKRF6UVibLMHcxATrSNrC6CdPmGwehK9doRk-2018-04-26 00:35:39.jpeg', NULL, '2018-04-25 17:35:39', 9, '2018-04-25 17:35:39', 9, 1, 0),
(79, 80, 18, 'srw1vEYm0rQFk3vDEo6uhfxLt6vsZHli7wkO6XWE-2018-04-26 00:35:40.jpeg', NULL, '2018-04-25 17:35:40', 9, '2018-04-25 17:35:40', 9, 0, 0);
...
INSERT INTO `Owner` (`Id`, `Name`, `Phone`, `Address`, `Uuid`, `Logo`, `CreatedDt`, `CreatedBy`, `UpdatedDt`, `UpdatedBy`) VALUES
(1, 'PT Kangkoding', '085691108802', 'Jl Bambu Raya No 3-5', 'f1c65761-7ab4-485b-8d2b-1fd0e56c621f', 'zQIzxLJO5D3nsVbgw1tGEm3TXS7Jp3Vh1YGzbj1E-2018-04-25 10:30:28.png', '2018-04-25 10:30:28', 0, '0000-00-00 00:00:00', 0);
...
INSERT INTO `Product` (`Id`, `_Owner`, `Name`, `Description`, `_Category`, `_Gender`, `Price`, `CreatedDt`, `CreatedBy`, `UpdatedDt`, `UpdatedBy`, `Status`, `Uuid`) VALUES
(1, 1, 'Lena Veil', 'Keterangan jilbab Lena Veil', 1, 2, 199000, '2018-03-29 13:14:09', 1, NULL, NULL, 1, '90546aed-03b9-4bde-ba38-532f1e7763a7'),
(2, 1, 'Jilbab Syar\'i Segi4 LAV', 'Keterangan jilbab Jilbab Syar\'i Segi4 LAV', 1, 2, 118500, '2018-03-10 17:03:23', 1, NULL, NULL, 1, 'd3f4967b-1e43-4472-8047-5481e2a67533'),
(4, 1, 'Kirana Khimar', 'Keterangan jilbab Kirana Khimar', 1, 2, 245000, '2018-03-29 13:14:29', 0, NULL, NULL, 1, 'b2dbcc50-7e87-42a8-9581-0ec8afc4af2c'),
(7, 1, 'Hannah Veil', 'Keterangan jilbab Hannah Veil', 1, 2, 299000, '2018-03-29 13:14:40', 0, NULL, NULL, 1, '67113cc9-09a0-45ab-a167-d61f12045453'),
(8, 1, 'Khimar Cherry Blossom', 'Keterangan jilbab Khimar Cherry Blossom', 1, 3, 600000, '2018-04-01 15:30:00', 0, NULL, NULL, 1, '315261a9-92e8-4b2a-a8a7-d130fa6ab435'),
(9, 1, 'Khimar Dired Leaves', 'Keterangan jilbab Khimar Dired Leaves', 1, 2, 600000, '2018-03-29 13:14:58', 0, NULL, NULL, 1, 'cfe06cd9-cfc6-40e5-abbf-d4163eeae0e7'),
(10, 1, 'Khimar Hydrangea', 'Keterangan jilbab Khimar Hydrangea', 1, 2, 600000, '2018-03-29 13:15:11', 0, NULL, NULL, 1, 'd700edbe-ceac-4753-99c4-879c8a2137e0'),
(11, 1, 'Salma Veil', 'Keterangan jilbab Salma Veil', 1, 2, 72500, '2018-03-29 13:15:20', 0, NULL, NULL, 1, '47d1e335-9bc4-419b-9920-310828d1b54c'),
(12, 1, 'Radhiya Khimar Syar\'i', 'Keterangan jilbab Radhiya Khimar Syar\'i', 1, 2, 345000, '2018-03-29 13:15:32', 0, NULL, NULL, 1, 'b39b744b-5a44-4f64-aa6d-ece579103162'),
(13, 1, 'Adzra Khimar Syar\'i', 'Keterangan jilbab Adzra Khimar Syar\'i', 1, 2, 325000, '2018-03-29 13:15:42', 0, NULL, NULL, 1, 'f2ebb690-9799-4ffe-b284-2a070012ac52'),
(14, 1, 'Moi Instant Shawl', 'Keterangan jilbab Moi Instant Shawl', 1, 2, 119000, '2018-03-29 13:15:50', 0, NULL, NULL, 1, '7731f939-7ecf-41f9-8ab8-7c7e3cf965c3'),
(15, 1, 'Khimar Lavenia', 'Keterangan Jilbab Khimar Lavenia', 1, 2, 600000, '2018-03-29 13:16:11', 0, NULL, NULL, 1, 'f98255de-5605-46ef-8469-7d892709baab'),
(16, 1, 'Malika Khimar Syar\'i', 'Keterangan jilbab Malika Khimar Syar\'i', 1, 2, 245000, '2018-03-29 13:16:19', 0, NULL, NULL, 1, '2b84326d-65db-427f-8b30-6793756f9584'),
(17, 1, 'Khimar Syaimah', 'Keterangan jilbab Khimar Syaimah', 1, 2, 950000, '2018-03-29 13:16:29', 0, NULL, NULL, 1, '0675baa7-1e89-4741-99cf-5a20f7c979e7'),
(18, 1, 'Khimar Aisyah 2', 'Keterangan jilbab Khimar Aisyah 2', 1, 2, 750000, '2018-03-29 13:16:41', 0, NULL, NULL, 1, '445c888a-abe8-44df-af33-37ddfef8adef'),
(21, 1, 'Ayyasa Khimar', 'Keterangan jilbab Ayyasa Khimar', 1, 2, 275000, '2018-03-29 13:16:48', 0, NULL, NULL, 1, '89a5c367-28ee-4953-bb53-a72d3d9c3e9b'),
(22, 1, 'Chesa Khimar', 'Keterangan jilbab Chesa Khimar', 1, 2, 240000, '2018-03-29 13:16:56', 0, NULL, NULL, 1, 'aac0ea02-d544-478b-91be-2f67dbbb6f41'),
(23, 1, 'Afifa Veil', 'Keterangan jilbab Afifa Veil', 1, 2, 185000, '2018-03-29 13:17:03', 0, NULL, NULL, 1, 'b5ae3c95-8638-473f-b571-95f920802b66'),
(24, 1, 'Zahra Veil', 'Keterangan jilbab Zahra Veil', 1, 2, 185000, '2018-03-29 13:17:09', 0, NULL, NULL, 1, '43c61454-d975-4d81-bd6b-ba632d80a673'),
(25, 1, 'Azia Veil', 'Keterangan jilbab Azia Veil', 1, 2, 180000, '2018-03-29 13:17:17', 0, NULL, NULL, 1, '5c017cf8-c5bb-4c15-a535-bc8735f7ae94'),
(26, 1, 'Saila Veil', 'Keterangan jilbab Saila Veil', 1, 2, 185000, '2018-03-29 13:17:26', 0, NULL, NULL, 1, '2e3c5fa9-e6ef-47ee-b5db-decee7d6dbf4'),
(27, 1, 'Hania Veil', 'Keterangan jilbab Hania Veil', 1, 2, 185000, '2018-03-29 13:17:32', 0, NULL, NULL, 1, '81ea37bd-79e2-4df3-acbd-21161d4130e8'),
(28, 1, 'Arwa Khimar Maroon', 'Keterangan jilbab Arwa Khimar Maroon', 1, 2, 267000, '2018-03-29 13:17:40', 0, NULL, NULL, 1, '9e45b220-e0a0-4f36-952e-eb86754d9b34'),
(72, 1, 'Gitar Gillmore GFX-11', 'Gitar ini keren sekali', 7, 3, 2000000, '2018-04-20 16:02:47', 9, '2018-04-20 16:02:47', 9, 1, '7c872f44-d7ba-4c21-92ea-c6f85ea55710'),
(73, 1, 'Sound system', 'Sound system ini keren sekali', 7, 3, 3000000, '2018-04-22 05:09:50', 9, '2018-04-22 05:09:50', 9, 1, '8654af0d-00b8-47df-8f56-942e35132033'),
(74, 1, 'Miska Soft Pink Bag', 'Tas ini keren sekali', 6, 2, 250000, '2018-04-22 09:13:55', 9, '2018-04-22 09:13:55', 9, 1, '70391216-3e7d-42c3-b1f0-1c06f513f4ab'),
(75, 1, 'Miska Pouch Akiko Polkadot', 'Tas hitam ini bagus sekali untuk wanita', 6, 2, 200000, '2018-04-22 09:16:50', 9, '2018-04-22 09:16:50', 9, 1, '46c6bb51-b017-4eab-a41d-5dd21122760d'),
(76, 1, 'Small Pouch Black', 'Tas hitam ini keren sekali untuk wanita zaman now', 6, 2, 200000, '2018-04-22 09:19:36', 9, '2018-04-22 09:19:36', 9, 1, 'abdf804b-ed02-4815-8cfe-f55aff6a3269'),
(77, 1, 'Small Pouch Polkadot Navy', 'Tas polkadot warna biru ini keren sekali untuk wanita zaman now', 6, 2, 200000, '2018-04-22 09:24:22', 9, '2018-04-22 09:24:22', 9, 1, '8d65e061-05f9-429f-91b9-c224f5b474f6');
...
CREATE TABLE `PurchaseOrder` (
  ...
  `_Owner` int(11) NOT NULL DEFAULT '1',
  `_Account` int(11) NOT NULL,
  `Fullname` varchar(100) NOT NULL,
  `_Province` int(11) NOT NULL,
  `_City` int(11) NOT NULL,
  ...
  `isVoucher` tinyint(1) DEFAULT NULL,
  `_Voucher` int(11) DEFAULT NULL,
  `status` char(1) NOT NULL DEFAULT '0',
  `CreatedDt` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `ExpiredDt` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
INSERT INTO `PurchaseOrder` (`OrderCode`, `_Owner`, `_Account`, `Fullname`, `_Province`, `_City`, `_District`, `Address`, `PostCode`, `Phone`, `Price`, `TransactionCode`, `FinalPrice`, `isVoucher`, `_Voucher`, `status`, `CreatedDt`, `ExpiredDt`) VALUES
('13QR8C620180426', 1, 8, 'Hadron user satu', 31, 3171, 3171100, 'Jl KH Royani 1 No 5', '12940', '085691108802', 200000, '024', 200024, NULL, NULL, '0', '2018-04-26 03:53:30', '2018-04-27 03:53:30'),
('14LMKBR20180423', 1, 6, 'Anton Paijol', 31, 3171, 3171100, 'Jl KH Royani 1 No 5', '12940', '085691108802', 1315500, '895', 1316395, NULL, NULL, '0', '2018-04-25 11:42:13', '2018-04-26 11:00:00'),
('1CH6O1920180426', 1, 8, 'Hadron user satu', 31, 3171, 3171100, 'Jl KH Royani 1 No 5', '12940', '085691108802', 652000, '534', 652534, NULL, NULL, '4', '2018-04-29 21:49:31', '2018-04-25 22:50:51'),
('1F0F5HH20180429', 1, 8, 'Hadron user satu', 32, 3201, 3201250, 'Bukit Dago Blok D4 No 23', '12940', '085691108802', 680000, '131', 680131, NULL, NULL, '0', '2018-04-29 14:48:45', '2018-04-30 21:48:45'),
('1FON1Y620180423', 1, 6, 'Budi Santoso', 31, 3171, 3171100, 'Jl KH Royani 1 No 5', '12940', '085691108802', 3134000, '093', 3134000, NULL, NULL, '2', '2018-04-26 07:54:52', '2018-04-26 08:00:00'),
('1PRJ78J20180426', 1, 8, 'Hadron user satu', 31, 3171, 3171100, 'Jl KH Royani 1 No 5', '12940', '085691108802', 534000, '747', 534747, NULL, NULL, '5', '2018-04-26 17:46:28', '2018-04-27 10:56:37');
...
CREATE TABLE `PurchaseOrderDetail` (
  `Id` int(11) NOT NULL,
  `_PurchaseOrder` varchar(20) NOT NULL,
  `_Product` int(11) NOT NULL,
  `_Color` int(11) NOT NULL,
  `_Size` int(11) NOT NULL,
  `isDiscount` tinyint(1) DEFAULT '0',
  `_Discount` int(11) DEFAULT NULL,
...
INSERT INTO `PurchaseOrderDetail` (`Id`, `_PurchaseOrder`, `_Product`, `_Color`, `_Size`, `isDiscount`, `_Discount`, `Total`, `Price`, `TotalPrice`, `CreatedDt`) VALUES
(1, '1FON1Y620180423', 2, 1, 5, 1, 1, 1, 108500, 108500, '2018-04-23 06:22:33'),
(2, '1FON1Y620180423', 2, 12, 5, 1, 1, 3, 108500, 325500, '2018-04-23 06:22:33'),
(3, '1FON1Y620180423', 73, 6, 5, 1, 25, 1, 2700000, 2700000, '2018-04-23 06:22:33'),
(4, '14LMKBR20180423', 2, 1, 5, 1, 1, 1, 108500, 108500, '2018-04-23 08:38:56'),
(5, '14LMKBR20180423', 2, 13, 5, 1, 1, 2, 108500, 217000, '2018-04-23 08:38:56'),
(6, '14LMKBR20180423', 8, 5, 8, 1, 2, 1, 540000, 540000, '2018-04-23 08:38:56'),
(7, '14LMKBR20180423', 74, 11, 8, 0, NULL, 1, 250000, 250000, '2018-04-23 08:38:56'),
(8, '14LMKBR20180423', 77, 2, 8, 0, NULL, 1, 200000, 200000, '2018-04-23 08:38:56'),
(9, '1CH6O1920180426', 26, 3, 8, 0, NULL, 1, 185000, 185000, '2018-04-26 03:50:51'),
(10, '1CH6O1920180426', 28, 21, 8, 0, NULL, 1, 267000, 267000, '2018-04-26 03:50:51'),
(11, '1CH6O1920180426', 77, 2, 8, 0, NULL, 1, 200000, 200000, '2018-04-26 03:50:51'),
(12, '13QR8C620180426', 77, 2, 8, 0, NULL, 1, 200000, 200000, '2018-04-26 03:53:30'),
(13, '1PRJ78J20180426', 28, 21, 8, 0, NULL, 2, 267000, 534000, '2018-04-26 03:56:37'),
(14, '1F0F5HH20180429', 22, 12, 8, 0, NULL, 2, 240000, 480000, '2018-04-29 14:48:45'),
(15, '1F0F5HH20180429', 77, 2, 8, 0, NULL, 1, 200000, 200000, '2018-04-29 14:48:45');
...
INSERT INTO `Stock` (`Id`, `_Product`, `_Color`, `_Size`, `Total`, `Note`, `CreatedDt`, `CreatedBy`, `UpdatedDt`, `UpdatedBy`) VALUES
(1, 2, 1, 5, 200, NULL, NULL, NULL, NULL, NULL),
(2, 2, 1, 4, 210, NULL, NULL, NULL, NULL, NULL),
(3, 2, 1, 3, 220, NULL, NULL, NULL, NULL, NULL),
(4, 2, 12, 5, 100, NULL, NULL, NULL, NULL, NULL),
(5, 2, 6, 5, 110, NULL, NULL, NULL, NULL, NULL),
(6, 2, 10, 5, 120, NULL, NULL, NULL, NULL, NULL),
(7, 2, 13, 5, 130, NULL, NULL, NULL, NULL, NULL),
(8, 1, 14, 8, 100, NULL, NULL, NULL, NULL, NULL),
(9, 4, 14, 8, 200, NULL, NULL, NULL, NULL, NULL),
(10, 7, 3, 8, 200, NULL, NULL, NULL, NULL, NULL),
(11, 8, 5, 8, 200, NULL, NULL, NULL, NULL, NULL),
(12, 9, 2, 8, 200, NULL, NULL, NULL, NULL, NULL),
(13, 10, 5, 8, 200, NULL, NULL, NULL, NULL, NULL),
(14, 11, 18, 8, 200, NULL, NULL, NULL, NULL, NULL),
(15, 12, 2, 8, 200, NULL, NULL, NULL, NULL, NULL),
(16, 13, 3, 8, 200, NULL, NULL, NULL, NULL, NULL),
(17, 14, 6, 8, 200, NULL, NULL, NULL, NULL, NULL),
(18, 15, 14, 8, 200, NULL, NULL, NULL, NULL, NULL),
(19, 16, 11, 8, 200, NULL, NULL, NULL, NULL, NULL),
(20, 16, 6, 8, 200, NULL, NULL, NULL, NULL, NULL),
(21, 16, 3, 8, 200, NULL, NULL, NULL, NULL, NULL),
(22, 17, 15, 8, 200, NULL, NULL, NULL, NULL, NULL),
(23, 18, 16, 8, 200, NULL, NULL, NULL, NULL, NULL),
(24, 21, 17, 8, 200, NULL, NULL, NULL, NULL, NULL),
(25, 22, 12, 8, 200, NULL, NULL, NULL, NULL, NULL),
(26, 23, 19, 8, 200, NULL, NULL, NULL, NULL, NULL),
(27, 24, 11, 8, 200, NULL, NULL, NULL, NULL, NULL),
(28, 25, 20, 8, 200, NULL, NULL, NULL, NULL, NULL),
(29, 26, 3, 8, 200, NULL, NULL, NULL, NULL, NULL),
(30, 27, 11, 8, 200, NULL, NULL, NULL, NULL, NULL),
(31, 28, 21, 8, 200, NULL, NULL, NULL, NULL, NULL),
(32, 72, 17, 5, 50, NULL, NULL, NULL, NULL, NULL),
(33, 73, 6, 5, 10, NULL, NULL, NULL, NULL, NULL),
(34, 74, 11, 8, 50, NULL, NULL, NULL, NULL, NULL),
(35, 75, 2, 8, 50, NULL, NULL, NULL, NULL, NULL),
(36, 76, 6, 8, 50, NULL, NULL, NULL, NULL, NULL),
(37, 77, 2, 8, 50, NULL, NULL, NULL, NULL, NULL);
...
CREATE TABLE `Subscription` (
  `Id` int(10) UNSIGNED NOT NULL,
  `_Owner` int(11) NOT NULL,
  `FromDt` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
INSERT INTO `_Bank` (`Id`, `Code`, `Name`, `Path`) VALUES
(1, '002', 'BANK BRI', 'Bank-BRI.png'),
(2, '008', 'BANK MANDIRI', 'Bank-Mandiri.png'),
(3, '009', 'BANK BNI', 'Bank-BNI.png'),
(4, '011', 'BANK DANAMON', NULL),
(5, '013', 'PERMATA BANK', NULL),
(6, '014', 'BANK BCA', 'Bank-BCA.png'),
(7, '016', 'BANK BII MAYBANK', NULL),
(8, '022', 'BANK CIMB NIAGA', 'Bank-CIMB-Niaga.png'),
(9, '026', 'BANK LIPPO', NULL),
(10, '028', 'BANK OCBC NISP', NULL),
(11, '031', 'CITIBANK', NULL),
(12, '147', 'BANK MUAMALAT', NULL),
(13, '153', 'BANK SINARMAS', NULL),
(14, '200', 'BANK TABUNGAN NEGARA (BTN)', NULL),
(15, '213', 'BANK TABUNGAN PENSIUNAN NASIONAL (BTPN)', NULL),
(16, '422', 'BANK BRI SYARIAH', NULL),
(17, '426', 'BANK MEGA', NULL),
(18, '427', 'BANK BNI SYARIAH', NULL),
(19, '441', 'BANK BUKOPIN', NULL),
(20, '451', 'BANK SYARIAH MANDIRI', NULL),
(21, '536', 'BANK BCA SYARIAH', NULL),
(22, '789', 'INDOSAT DOMPETKU', NULL),
(23, '911', 'TELKOMSEL TCASH', NULL),
(24, '950', 'BANK COMMONWEALTH', NULL);
...
INSERT INTO `_Category` (`Id`, `_Owner`, `Name`, `CreatedDt`, `CreatedBy`, `UpdatedDt`, `UpdatedBy`) VALUES
(1, 1, 'Jilbab', NULL, NULL, NULL, NULL),
(2, 1, 'Pakaian', NULL, NULL, NULL, NULL),
(3, 1, 'Gamis', NULL, NULL, NULL, NULL),
(5, 1, 'Sepatu', NULL, NULL, NULL, NULL),
(6, 1, 'Tas', NULL, NULL, NULL, NULL),
(7, 1, 'Alat Musik', '2018-04-20 11:52:57', NULL, '2018-04-20 11:52:57', NULL),
(8, 1, 'Transportasi', '2018-04-22 12:08:38', NULL, '2018-04-22 12:08:38', NULL);
...
CREATE TABLE `_City` (
  ...
  `_Province` char(2) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
...
INSERT INTO `_City` (`Id`, `_Province`, `Name`) VALUES
...
INSERT INTO `_Color` (`Id`, `_Owner`, `Name`, `CreatedDt`, `CreatedBy`, `UpdatedDt`, `UpdatedBy`) VALUES
(1, 1, 'Red', NULL, NULL, '2018-04-04 17:00:00', NULL),
(2, 1, 'Blue', NULL, NULL, NULL, NULL),
(3, 1, 'Purple', NULL, NULL, NULL, NULL),
(4, 1, 'Jingga', NULL, NULL, NULL, NULL),
(5, 1, 'White', NULL, NULL, NULL, NULL),
(6, 1, 'Black', NULL, NULL, NULL, NULL),
(7, 1, 'Green', NULL, NULL, NULL, NULL),
(8, 1, 'Tosca', NULL, NULL, NULL, NULL),
(9, 1, 'Nila', NULL, NULL, NULL, NULL),
(10, 1, 'Milo', NULL, NULL, NULL, NULL),
(11, 1, 'Pink', NULL, NULL, NULL, NULL),
(12, 1, 'Lavender', NULL, NULL, NULL, NULL),
(13, 1, 'Blue Pastel', NULL, NULL, NULL, NULL),
(14, 1, 'Nude', NULL, NULL, NULL, NULL),
(15, 1, 'Mint Green', NULL, NULL, NULL, NULL),
(16, 1, 'Blue Denim', NULL, NULL, NULL, NULL),
(17, 1, 'Brown', NULL, NULL, NULL, NULL),
(18, 1, 'Yellow', NULL, NULL, NULL, NULL),
(19, 1, 'Dark Grey', NULL, NULL, NULL, NULL),
(20, 1, 'Grey', NULL, NULL, NULL, NULL),
(21, 1, 'Maroon', NULL, NULL, NULL, NULL),
(22, 1, 'Warna baru', '2018-04-22 12:08:27', NULL, '2018-04-22 12:08:27', NULL);

-- --------------------------------------------------------

--
-- Table structure for table `_DiscountType`
--

CREATE TABLE `_DiscountType` (
  `Id` tinyint(4) NOT NULL,
  `Type` varchar(50) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `_DiscountType`
--

INSERT INTO `_DiscountType` (`Id`, `Type`) VALUES
(1, 'Percent'),
(2, 'Price');
...
CREATE TABLE `_District` (
  ...
  `_City` char(4) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `Name` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
INSERT INTO `_District` (`Id`, `_City`, `Name`) VALUES
...
INSERT INTO `_Gender` (`Id`, `Name`) VALUES
(1, 'Male'),
(2, 'Female'),
(3, 'Male & Female');
...
--
-- Dumping data for table `_Province`
--

INSERT INTO `_Province` (`Id`, `Name`) VALUES
('11', 'ACEH'),
('12', 'SUMATERA UTARA'),
('13', 'SUMATERA BARAT'),
('14', 'RIAU'),
...
--
-- Table structure for table `_Size`
--
...
--
-- Dumping data for table `_Size`
--

INSERT INTO `_Size` (`Id`, `Size`) VALUES
(1, 'XXL'),
(2, 'XL'),
(3, 'L'),
(4, 'M'),
...
ALTER TABLE `Discount`
  ADD PRIMARY KEY (`Id`),
  ADD KEY `_DiscountType` (`_DiscountType`),
  ADD KEY `_Product` (`_Product`);
...
ALTER TABLE `Product`
  ADD PRIMARY KEY (`Id`),
  ADD UNIQUE KEY `indexName` (`Name`),
  ADD UNIQUE KEY `indexDescription` (`Description`);
...
ALTER TABLE `PurchaseOrder`
  ...
  ADD KEY `_Account` (`_Account`),
  ADD KEY `_Province` (`_Province`),
  ADD KEY `_City` (`_City`),
  ADD KEY `_District` (`_District`),
  ADD KEY `_Voucher` (`_Voucher`),
  ADD KEY `_Owner` (`_Owner`);
...
ALTER TABLE `PurchaseOrderDetail`
  ADD PRIMARY KEY (`Id`),
  ADD KEY `_PurchaseOrder` (`_PurchaseOrder`),
  ADD KEY `_Product` (`_Product`),
  ADD KEY `_Color` (`_Color`),
  ADD KEY `_Size` (`_Size`),
  ADD KEY `_Discount` (`_Discount`);
...
ALTER TABLE `_City`
  ADD PRIMARY KEY (`Id`),
  ADD KEY `indonesia_cities_province_id_foreign` (`_Province`);
...
--
-- Indexes for table `_DiscountType`
--
ALTER TABLE `_DiscountType`
  ADD PRIMARY KEY (`Id`);
...
ALTER TABLE `_District`
  ADD PRIMARY KEY (`Id`),
  ADD KEY `indonesia_districts_city_id_foreign` (`_City`);

--
-- Indexes for table `_Gender`
--
ALTER TABLE `_Gender`
  ADD PRIMARY KEY (`Id`);

--
-- Indexes for table `_Model`
--
ALTER TABLE `_Model`
  ADD PRIMARY KEY (`Id`);
...
--
-- AUTO_INCREMENT for table `_Model`
--
ALTER TABLE `_Model`
  MODIFY `Id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT;
COMMIT;
